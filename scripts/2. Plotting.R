...
df = data.frame(untreat_common = c(nrow(UCommon %>% count(seqnames,start)),
                                   nrow(UCommon %>% count(annot.seqnames,annot.start)),
                                   0,
                                   0
                                   ),
                treat_common = c(0,
                                 0,
                                 nrow(TCommon %>% count(seqnames,start)),
                                 nrow(TCommon %>% count(annot.seqnames,annot.start))
                                 ),
...
df = df %>% pivot_longer(cols = !condition,names_to = "name",values_to = "value")

ggplot(df,aes(y=condition,x=value,fill= name)) + 
  geom_bar(stat = "identity", position = "stack") +
  labs(x = "Number of peaks",y="",fill = "Condition") +
  theme_bw() +
  scale_fill_manual(values=c("#A9ADB0","#4C5073","#FADFAC"))
